...
// }
?>
  <i style="color:white; position: relative; margin:auto;padding: 10px;top:8px;">Virtual FCHacks</i>
  <script>
  ...
  <div style="text-align:right; color:white; position: relative; margin:auto;padding-right: 3px;top:-16px;"> Logged in as: 
  <?php 
 ...
 if (!isset($_SESSION["username"]) && !isset($_SESSION["Jusername"])) {
	if (strpos(getcwd(), 'participants') !== false) {
		echo "<a href='/participants/login.php'>Login</a>";
	} else if (strpos(getcwd(), 'judges') !== false) {
		echo "<a  href='/judges/login.php'>Login</a>";
	} else {
		echo "<a  href='/participants/login.php'>Login</a>";
	}
 } else {
	echo '; <a  href="/participants/logout.php"> Logout </a>';
 }
